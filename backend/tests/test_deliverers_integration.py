...
def test_api_assignment_marks_order_and_deliverer():
    ...
    created = client.post(
        '/api/deliverers/', json={'name': 'Ana', 'phone': '11999999999', 'region': 'Zona Sul'})
    assert created.status_code == 201

    assigned = client.post(
        '/api/orders/assign/', json={'order_id': str(uuid4()), 'region': 'Zona Sul'})
...
def test_api_reassignment_uses_other_available_deliverer():
    ...
    first = client.post(
        '/api/deliverers/', json={'name': 'Ana', 'phone': '11999999999', 'region': 'Zona Sul'})
    second = client.post(
        '/api/deliverers/', json={'name': 'Bruno', 'phone': '11988888888', 'region': 'Zona Sul'})
    order_id = str(uuid4())

    assigned = client.post('/api/orders/assign/', json={
                           'order_id': order_id, 'region': 'Zona Sul', 'deliverer_id': first.json()['id']})
    assert assigned.status_code == 200

    reassigned = client.post(
        f'/api/orders/{order_id}/reassign/', json={'reason': 'refused'})
